...
const METADATA_COLLECTION_ID = import.meta.env
  .VITE_APPWRITE_REPORT_METADATA_COLLECTION_ID;
...
const Metadata = () => {
  ...
  return (
    <div className="p-6">
      ...
      <div className="overflow-x-auto bg-white bg-gray-800 rounded-lg shadow">
        {loading ? (
          <p className="p-4 text-gray-500 text-gray-300">Loading...</p>
        ) : metadata.length === 0 ? (
          <p className="p-4 text-gray-500 text-gray-300">No metadata found.</p>
        ) : (
          <table className="min-w-full divide-y divide-gray-200 divide-gray-700">
            <thead className="bg-gray-50 bg-gray-700">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 text-gray-300 uppercase">
                  Key
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 text-gray-300 uppercase">
                  Value
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 text-gray-300 uppercase">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="bg-white bg-gray-800 divide-y divide-gray-200 divide-gray-700">
              {metadata.map((item) => (
                <tr key={item.$id}>
                  <td className="px-6 py-4 text-sm text-gray-900 text-white">
                    {item.key || "—"}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-700 text-gray-300">
                    {item.value || "—"}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-700 text-gray-300">
                    {item.report_id || "—"}
                  ...
                  <td className="px-6 py-4 text-right">
                    <button
                      ...
                      className="text-red-600 hover:text-red-900 text-red-400 hover:text-red-300"
                      title="Delete"
                    >
