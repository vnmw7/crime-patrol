...
const SUSPECTS_COLLECTION_ID = import.meta.env.VITE_APPWRITE_REPORT_SUSPECTS_COLLECTION_ID;
...
const Suspects = () => {
  ...
  const fetchSuspects = async () => {
    try {
      const response = await databases.listDocuments(DATABASE_ID, SUSPECTS_COLLECTION_ID);
  ...
  const handleDelete = async (suspectId) => {
    const confirmDelete = confirm("Are you sure you want to delete this suspect?");
    if (!confirmDelete) return;

    try {
      await databases.deleteDocument(DATABASE_ID, SUSPECTS_COLLECTION_ID, suspectId);
      setSuspects((prev) => prev.filter((suspect) => suspect.$id !== suspectId));
    } catch (error) {
  ...
  return (
    <div className="p-6">
      ...
      {loading ? (
        ...
        <p className="text-gray-500 dark:text-gray-400">No suspect records found.</p>
      ) : (
          ...
          <table className="min-w-full bg-white dark:bg-gray-800 rounded shadow">
            ...
            <tbody>
              {suspects.map((suspect) => (
                <tr key={suspect.$id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
