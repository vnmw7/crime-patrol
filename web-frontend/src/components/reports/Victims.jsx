...
const VICTIMS_COLLECTION_ID = import.meta.env.VITE_APPWRITE_REPORT_VICTIMS_COLLECTION_ID;
...
const Victims = () => {
  ...
  const fetchVictims = async () => {
    try {
      const response = await databases.listDocuments(DATABASE_ID, VICTIMS_COLLECTION_ID);
  ...
  const handleDelete = async (victimId) => {
    const confirmDelete = confirm("Are you sure you want to delete this victim entry?");
    if (!confirmDelete) return;

    try {
      await databases.deleteDocument(DATABASE_ID, VICTIMS_COLLECTION_ID, victimId);
  ...
  return (
    <div className="p-6">
      ...
      {loading ? (
        ...
        <p className="text-gray-500 dark:text-gray-400">No victim records found.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white dark:bg-gray-800 rounded shadow">
            ...
            <tbody>
              {victims.map((victim) => (
                <tr key={victim.$id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
